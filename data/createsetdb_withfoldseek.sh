...
if [ "$("${MMSEQS}" dbtype "${TMP_PATH}/seqDB")" = "Nucleotide" ]; then
    ...
    if notExists "${TMP_PATH}/seqDB_nucl.index"; then
        # shellcheck disable=SC2086
    ...
    if notExists "${TMP_PATH}/seqDB.index"; then
        # shellcheck disable=SC2086
        "${MMSEQS}" translatenucs "${TMP_PATH}/seqDB_nucl" "${TMP_PATH}/seqDB" ${TRANSLATENUCS_PAR} \
...
if [ -n "${USE_STRUCTURES}" ]; then
    if notExists "${TMP_PATH}/foldseekrefDB.index"; then
        # shellcheck disable=SC2086
        "${FOLDSEEK}" databases Alphafold/UniProt-NO-CA "${TMP_PATH}/foldseekrefDB" "${TMP_PATH}/databases" \
            || fail "download foldseek database failed"
    fi

    if notExists "${OUTDB}.index"; then
        # shellcheck disable=SC2086
        "${MMSEQS}" aa2foldseek "${TMP_PATH}/seqDB" "${TMP_PATH}/foldseekrefDB" "${OUTDB}" ${AA2FOLDSEEK_PAR} \
            || fail "aa2foldseek failed"
    fi
else
	if notExists "${OUTDB}.index"; then
	    # shellcheck disable=SC2086
